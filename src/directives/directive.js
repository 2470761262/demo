...
export default {
  install(Vue) {
      ...
      bind(el, binding, vnode) {
        ...
        vnode.context.$nextTick(() => {
          if (typeof binding.value == "object") {
            ...
            scroll = function() {
              let offsetParent = document.querySelector(
                "." + binding.value.offsetParent
              );
              let parentOffset = util.getElementTop(offsetParent);
              if (!parentOffset) return false;
              parentOffset =
                parent.clientHeight - (parentOffset - (binding.value.top || 0));
              let elHeight = el.clientHeight / 2;
              let overHide =
                util.getElementTop(offsetParent) -
                parent.scrollTop -
                (binding.value.top || 0);
              ...
              el.style.cssText = `top:${(parentOffset + parent.scrollTop) / 2 -
                elHeight +
                overHide}px`;
            };
          } else {
            ...
            scroll = function() {
              el.style.cssText = `top:${parent.clientHeight / 2 +
                parent.scrollTop -
    ...
    Vue.filter(
      "emptyReadZero",
      (value, afterCompany = "", beforeCompany = "") => {
