        ...
        <el-table-column label="操作"
          ...
          <template v-slot="scope">
            <el-button type="primary"
                       ...
                       @click="Reply(scope.row.accounId,scope.row.accountName,scope.row.functionPoint,scope.row.pics,scope.row.id)">答复</el-button>
          </template>
        </el-table-column>
      </template>
    </list-page>
    <el-dialog title="给予答复"
               :visible.sync="dialogVisible"
               width="40%">
      <div>
        {{accountName}}的反馈：{{message}}
      </div>
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="updateReply()">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    Reply (aid, name, msg, pic, id) {//打开弹窗
