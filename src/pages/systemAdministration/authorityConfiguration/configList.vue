    ...
    <list-page
      ...
      <template v-slot:top>
        <div class="query-cell">
          ...
          <el-button type="primary" style="margin-left:10px" size="mini" @click="queryCompanyByParams" >查询</el-button>
        </div>
      </template>
...
<script>
  ...
  let array = [
    { name: "岗位设置", isType: "1",style:"primary", methodName: "showPositionList" },
  ];
  ...
  export default {
    ...
    data() {
      return {
        ...
        tableDataColumn: [
          ...
          { prop: "type", label: "类型",formater:function (row) {
    ...
    methods: {
      ...
      getOpeBtns(row) {
        if(row.type == "TREE_TYPE_POSITION"){
          return [
            { name: "角色设置", isType: "1",style:"danger", methodName: "showRoleDetail" },
          ];
        }else if(row.type == "TREE_TYPE_ROLE"){
          return [
            { name: "个人权限设置", isType: "1",style:"primary", methodName: "setEmployeeAuthority" },
            { name: "权限设置", isType: "1",style:"warning", methodName: "setPosition" },
          ];
        }else{
          console.log(row.parentId,"xxxxxx............");
          if(row.parentId == 0 && array.length == 1){
            array.push({ name: "设置权限", isType: "1",style:"primary", methodName: "editCompanyDetail" });
            console.log(array,"............");
          }
          return array;
        }
      ...
      showRoleDetail(row){
        this.$router.push({ path: "/sys/positionManager", query: { id: row.id } });
      },
      //个人权限设置
      ...
      showPositionList(e) {
        var that = this;
        //跳转页面
        that.$router.push({ path: '/sys/roleManagementList', query: { "id": e.id } });
      },
    }
  };
</script>
