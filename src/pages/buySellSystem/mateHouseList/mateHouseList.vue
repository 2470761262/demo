...
<template>
  <div>
    <div class="page-form-tips">
      Tips：请根据自己的房源需求，来匹配相应的房源，匹配项多选，不填则视为该选项不查询；
    </div>
    <el-form
      ref="form"
    ...
    >
      <!-- 楼盘 -->
      <div v-for="(item, index) in mate" :key="index">
        ...
        <div class="page-form-inline">
          <el-form-item label="inArea："> {{ item.inArea }}㎡ </el-form-item>
          <el-form-item label="price："> {{ item.price }}万元 </el-form-item>
          <el-form-item label="rooms："> {{ item.rooms }}室 </el-form-item>
        </div>
