...
export default {
  ...
  methods: {
    remoteCommunityNameChange (e) {
      ...
      this.selectPageCommunit.name = this.selectPageCommunit.list[findResultIndex].name;
      this.tableFor = [];
      this.tableForList = '';
      this.tableAvgPirce = '';
    ...
    remoteBuildingNo (e) {
      let that = this;
      //验证楼盘
      that.$validator.validate("communityId", this.selectPageCommunit.id).then((result) => {
        if (result) {
    ...
    remoteBuildingNoChange (e, index) {
      ...
      this.tableFor[index].buildingNo = this.selectPageBuilding.list[findResultIndex].name;
      //清空对应的房间
    ...
    houseListChange (e) {
      ...
      for (let i = 0; i < e; i++) {
        this.tableFor.push({
          price: !Number.isNaN(that.tableAvgPirce) ? (that.tableAvgPirce / e).toFixed(2) : '',
          buildingNo: '',
          buildingId: '',
          roomNo: '',
          roomId: '',
