    ...
    <template>
      ...
      <div class="replace-image-list-div">
        ...
        >
          ...
          <i class="el-icon-close icon" @click="removeImg(item)"></i>
        </div>
      </div>
      ...
      this.$validator.validateAll().then(e => {
        if (e) {
          let url = `/agentHouse/propertyCheck/${
            this.replaceType == 1 ? "insertApplyFor" : "insertReplace"
          }`;
          let params = {
            Eid: this.houseId.id,
      ...
      that.$api
        .post({
          ...
          data: {
            ...
            businessParams: JSON.stringify({ test: "委托人" })
          },
