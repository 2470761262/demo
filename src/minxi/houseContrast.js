...
export default {
    methods: {
        ...
        postUrl () {
            this.$router.push({ path: "/buySellSystem/agentHouseContrast", query: { houseId: this.$cookies.get("houseId") } });//获取cookie并跳转
        },
        addCookie (id) {
        ...
        isForBut (type) {
            ...
            if (this.$cookies.get("houseId") != null) {
                if (this.$cookies.get("houseId").split(",")[1] == undefined && this.$cookies.get("houseId") == typ) {
                    array = [
